...
public class P11Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P11, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P11_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P11_predecessor\"]/tuple/atom/@label", xml);
		...
		return build("P11", apps, configs, event, xml);
	}
