...
public class P12Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P12, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P12_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P12_predecessor\"]/tuple/atom/@label", xml);
		...
		Optional<CavTriple> p_cav = Optional.absent();			p_cav = Iterables.tryFind(p_TOr_Rules.get(0).acts, cav -> cav.atr.equals("mode") && cav.val.equals("Away"));
		if (!p_cav.isPresent())
		...
		return build("P12", apps, configs, event, xml);
	}
