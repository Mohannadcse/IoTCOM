...
public class P13Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P13, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P13_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P13_predecessor\"]/tuple/atom/@label", xml);
		...
		return build("P13", apps, configs, event, xml);
	}
