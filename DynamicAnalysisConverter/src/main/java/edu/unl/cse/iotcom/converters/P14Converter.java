...
public class P14Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P14, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P14_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P14_predecessor\"]/tuple/atom/@label", xml);
		...
		if (!p_cav.isPresent())
			p_cav = Iterables.tryFind(p_TOr_Rules.get(0).trgs,
					cav -> cav.atr.equals("motion") && cav.val.equals("active"));
		if (!p_cav.isPresent())
			p_cav = Iterables.tryFind(p_TOr_Rules.get(0).trgs,
					cav -> cav.atr.equals("presence") && cav.val.equals("present"));
		...
		return build("P14", apps, configs, event, xml);
	}
