...
public class P15Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P15, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P15_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P15_predecessor\"]/tuple/atom/@label", xml);
		...
		if (!p_cav.isPresent())
			p_cav = Iterables.tryFind(p_TOr_Rules.get(0).trgs,
					cav -> cav.atr.equals("contact") && cav.val.equals("open"));
		if (!p_cav.isPresent())
		...
		return build("P15", apps, configs, event, xml);
	}
