...
public class P16Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P16, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P16_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P16_predecessor\"]/tuple/atom/@label", xml);
		...
		if (!p_cav.isPresent())
			p_cav = Iterables.tryFind(p_TOr_Rules.get(0).trgs,
					cav -> cav.atr.equals("smoke") && cav.val.equals("detected"));
		if (!p_cav.isPresent())
		...
		return build("P16", apps, configs, event, xml);
	}
