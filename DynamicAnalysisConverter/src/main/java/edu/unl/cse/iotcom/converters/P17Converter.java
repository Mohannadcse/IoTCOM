...
public class P17Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P17, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P17_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P17_predecessor\"]/tuple/atom/@label", xml);
		...
		return build("P17", apps, configs, event, xml);
	}
