...
public class P1Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P1, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P1_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P1_predecessor\"]/tuple/atom/@label", xml);
		...
		if (!p_cav.isPresent()) {
			p_cav = Iterables.tryFind(p_TOr_Rules.get(0).trgs, t -> t.atr.equals("temperature") && t.val.equals("low"));
		}
		if (!p_cav.isPresent()) {
		...
		return build("P1", apps, configs, event, xml);
	}
