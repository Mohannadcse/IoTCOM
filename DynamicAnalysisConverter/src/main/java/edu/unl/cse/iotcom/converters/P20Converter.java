...
public class P20Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P20, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P20_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P20_predecessor\"]/tuple/atom/@label", xml);
		...
		return build("P20", apps, configs, event, xml);
	}
