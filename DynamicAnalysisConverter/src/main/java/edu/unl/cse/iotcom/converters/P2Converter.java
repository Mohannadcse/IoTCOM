...
public class P2Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P2, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P2_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P2_predecessor\"]/tuple/atom/@label", xml);
		...
		Set<CavTriple> event = Sets.newHashSet(Iterables.getFirst(p_TOr_Rules.get(0).trgs, null));
		...
		return build("P2", apps, configs, event, xml);
	}
