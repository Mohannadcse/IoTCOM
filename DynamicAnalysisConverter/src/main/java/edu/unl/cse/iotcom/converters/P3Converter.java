...
public class P3Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P3, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P3_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P3_predecessor\"]/tuple/atom/@label", xml);
		...
		Optional<CavTriple> p_cav = Iterables.tryFind(p_TOr_Rules.get(0).trgs,
				t -> t.atr.equals("contact") && t.val.equals("closed"));
		if (!p_cav.isPresent())
		...
		return build("P3", apps, configs, event, xml);
	}
