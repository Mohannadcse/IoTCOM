...
public class P4Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P4, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P4_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P4_predecessor\"]/tuple/atom/@label", xml);
		...
		return build("P4", apps, configs, event, xml);
	}
