...
public class P5Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P5, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P5_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P5_predecessor\"]/tuple/atom/@label", xml);
		...
		return build("P5", apps, configs, event, xml);
	}
