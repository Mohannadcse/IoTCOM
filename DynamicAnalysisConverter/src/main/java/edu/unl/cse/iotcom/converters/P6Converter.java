...
public class P6Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P6, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P6_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P6_predecessor\"]/tuple/atom/@label", xml);
		...
		if (!p_cav.isPresent())
			p_cav = Iterables.tryFind(p_TOr_Rules.get(0).trgs,
					cav -> cav.atr.equals("temperature") && cav.val.equals("high"));
		...
		return build("P6", apps, configs, event, xml);
	}
