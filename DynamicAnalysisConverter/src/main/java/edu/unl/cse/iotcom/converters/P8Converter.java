...
public class P8Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P8, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P8_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P8_predecessor\"]/tuple/atom/@label", xml);
		...
		return build("P8", apps, configs, event, xml);
	}
