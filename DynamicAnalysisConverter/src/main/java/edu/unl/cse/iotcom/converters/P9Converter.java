...
public class P9Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for P9, we want to find two skolems: r and predecessor
		String r_ = xpath.findStr("//skolem[@label=\"$P9_r\"]/tuple/atom/@label", xml);
		String p_ = xpath.findStr("//skolem[@label=\"$P9_predecessor\"]/tuple/atom/@label", xml);
		...
		return build("P9", apps, configs, event, xml);
	}
