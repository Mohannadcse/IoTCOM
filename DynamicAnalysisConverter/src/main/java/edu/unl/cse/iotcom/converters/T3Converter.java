...
public class T3Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for T3, we have two skolems: r and r'
		String r_ = xpath.findStr("//skolem[@label=\"$t3_r\"]/tuple/atom/@label", xml);
		String rp = xpath.findStr("//skolem[@label=\"$t3_r'\"]/tuple/atom/@label", xml);
		...
		return build("t3", apps, configs, event, xml);
	}
