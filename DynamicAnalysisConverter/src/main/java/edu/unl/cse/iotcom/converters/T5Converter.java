...
public class T5Converter extends ThreatConverter {

	@Override
	public String convert(Graph<String, DefaultEdge> connected, Node xml) throws Exception {
		// for T5, we want to find four skolems: a, a', r, and r'
		String a_ = xpath.findStr("//skolem[@label=\"$t5_a\"]/tuple/atom/@label", xml);
		String r_ = xpath.findStr("//skolem[@label=\"$t5_r\"]/tuple/atom/@label", xml);
		String ap = xpath.findStr("//skolem[@label=\"$t5_a'\"]/tuple/atom/@label", xml);
		String rp = xpath.findStr("//skolem[@label=\"$t5_r'\"]/tuple/atom/@label", xml);
		...
		return build("t5", apps, configs, shared, xml);
	}
}
